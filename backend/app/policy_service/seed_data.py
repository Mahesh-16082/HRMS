...
def seed_policies(db: Session) -> int:
    """
    Idempotently seeds the 20 predefined workplace policies into the database.
    Checks existing policies by title and only inserts missing ones.
    Returns the count of newly inserted policies.
    """
    inserted_count = 0
    for item in PREDEFINED_POLICIES:
        existing = db.query(Policy).filter(Policy.title == item["title"]).first()
        if not existing:
            policy = Policy(
                title=item["title"],
...
if __name__ == "__main__":
    db = SessionLocal()
    try:
        created = seed_policies(db)
        print(f"Policy seed completed. {created} new policies inserted.")
    finally:
        db.close()
